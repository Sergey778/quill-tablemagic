package tablemagic.magiccontext.postgresql.twitterfuture
...
class NoFutureTest extends FlatSpec with Matchers with NoFutureTestUtil {
  ...
  object Context {
    @MagicContext(DatabaseType.FinaglePostgres, FutureType.NoFuture)
    class PGFinagleContext

    implicit val pgfinagle = new PGFinagleContext[SnakeCase with Escape]("pgfinagle")
  }
