package tablemagic.magiccontext.postgresql.jdbc

import java.util.UUID

import io.getquill.context.sql.SqlContext
import io.getquill.{Escape, PostgresDialect, SnakeCase}
import org.scalatest.{FlatSpec, Matchers}
import tablemagic.context.conversion.BlockingNoFutureContext
import tablemagic.context.{DatabaseType, FutureType, MagicContext}
import tablemagic.testutil.NoFutureTestUtil


class NoFutureTest extends FlatSpec with Matchers with NoFutureTestUtil {
  object Context {
    @MagicContext(DatabaseType.Postgres, FutureType.NoFuture)
    class PGJdbcContext

    implicit val pgjdbc = new PGJdbcContext[SnakeCase with Escape]("pgjdbc")
  }
  ...
  import pgjdbc._
  "PostgresJDBC Context" should "be able to batch insert data into table" in {
  ...
  it should "be able to get all data from table" in {
    ...
    val quotedUsers: List[QuillTestUser] = genericRun(allQuery)
    val inlinedUsers: List[QuillTestUser] = genericRun(query[QuillTestUser])
  ...
  it should "be able to get filtered data from table" in {
    ...
    val quotedUsers: List[QuillTestUser] = genericRun(filteredQuery)
  ...
  it should "be able to partially update data in table" in {
    ...
    genericRun(checkQuery) shouldEqual 0
  }
  ...
  it should "be able to fully update data in table" in {
    ...
    genericRun(query[QuillTestUserToken].filter(_.userId == 1).size) shouldEqual 1
  }
